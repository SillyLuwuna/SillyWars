...
public class RtsActionUtils
{
	...
	private BaseStructure? GetNextStructure(WorldState state, StructureType type, Vec2Int center)
	{
		...
		int maxLength = Mathf.Max((int)map.Width, (int)map.Height);
		...
		for (int i = 0; i < maxLength; i++)
		{
			...
			int yStart = Mathf.Max(start.y, minValue.y);
			int xStart = Mathf.Max(start.x, minValue.x);
	...
	private Vec2 GetClosestWalkableTile(Vec2 pos, WorldState world)
	{
		...
		for (int i = 0; i < maxLength; i++)
		{
			...
			int yStart = Mathf.Max(start.y, 0);
			int xStart = Mathf.Max(start.x, 0);
